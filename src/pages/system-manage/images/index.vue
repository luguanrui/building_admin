    ...
    <div class="table-wrapper">
      <a-table :columns="columns" :data-source="data" :rowKey="(record, index) => index" :pagination="pagination" @change="handleChange" :loading="loading">
        ...
        <template slot="operation" slot-scope="record">
          ...
          <a-button size="small" @click="handlePublish(record)" v-if="record.isPublish === 0 && permissionList.includes('100092')">发布</a-button>
          <a-button size="small" @click="handleBack(record)"  v-if="record.isPublish === 1 && permissionList.includes('100092')">撤回</a-button>
        </template>
      </a-table>
    </div>
    <!-- 新增修改公告 -->
